...
def cellpose_parameters = ["diameter":diameter, "model":model, "net_avg":net_avg]

// Define preprocessing step, like having the right image for export. Here we will use channel 1
// This takes an ImagePlus as input and should return another ImagePlus that will be used for export
def preprocess = {image -> 
    return ChannelSplitter.split(image)[0]
}

// The virtualenv from which we will run python.exe
def cellpose_env_folder = 'D:\\environments\\cellpose'

// The location of the script that will process the images and return RoiSets
def cellpose_python_file = 'D:\\qupath-groovy-python-runner\\Examples\\CellPose\\cellpose_to_ij_rois.py'

// This line make sure that everything is ready to go
def pRCellPose = new PythonRoiRunner( cellpose_env_folder, cellpose_python_file)
                                    .setDownsample(2)
                                    .setParameters( cellpose_parameters )
                                    
// Process the currently selected object                  
pRCellPose.processRegion(getSelectedObject(), preprocess )
